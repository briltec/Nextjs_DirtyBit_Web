...
export const getStaticData = axios.create({
  baseURL: BASE_URL + "auth/getStaticData",
  headers: HEADERS,
});

getStaticData.interceptors.response.use(
  (response) => response,
  (error) => {
    const originalRequest = error.config;
    if (
      ...
      return refreshTokenApi
        ...
        .then((response) => {
          ...
          Cookies.set("refresh", refresh, { expires: 14 });
          HEADERS["Authorization"] = "JWT " + access;
          getStaticData.defaults.headers["Authorization"] = "JWT " + access;
          originalRequest.headers["Authorization"] = "JWT " + access;
          return getStaticData(originalRequest);
        })
        .catch((err) => {
          // LOGOUT AND REDIRECT TO SIGNIN AGAIN
          console.log(err);
